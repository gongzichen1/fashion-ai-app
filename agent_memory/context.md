...
- 后端已实现飞书 OAuth v2、微信 jscode2session、同源会话、SQLite/MySQL/PostgreSQL 用户隔离数据层、local/COS 图片存储适配器和图片到期清理 CLI。
...
- SQLite 与本地上传目录只用于本地/单实例 MVP；生产多实例必须通过 `DATABASE_URL` 使用托管 MySQL/PostgreSQL，并使用私有 COS。
...
- 飞书 `open_id` 映射为唯一 `external_id`；重复或并发免登必须复用同一内部用户。
