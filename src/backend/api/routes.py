...
def health_check():
    """健康检查接口"""
    return jsonify(
        {"success": True, "message": "服务运行正常", "timestamp": datetime.now().isoformat()}
    )
...
def analyze_image():
    ...
    try:
        ...
        if "image" in request.files:
            ...
            return jsonify({"success": False, "message": "请上传图片或提供base64编码的图片"}), 400
        ...
        recommendations = ai_service.generate_recommendations(
            analysis_result, style_preference
        )
        ...
        return jsonify({"success": True, "message": "分析成功", "data": result})
...
def get_recommendations():
    ...
    try:
        ...
        if not analysis_result:
            return jsonify({"success": False, "message": "请提供服装分析结果"}), 400
        ...
        return jsonify({"success": False, "message": f"推荐生成失败: {str(e)}"}), 500
...
def get_weather():
    ...
    try:
        ...
        if latitude is None or longitude is None:
            return (
                jsonify({"success": False, "message": "请提供 latitude 和 longitude"}),
                400,
            )

        if not (-90 <= latitude <= 90) or not (-180 <= longitude <= 180):
            return jsonify({"success": False, "message": "经纬度范围不合法"}), 400

        weather = _fetch_current_weather(latitude, longitude)

        return jsonify({"success": True, "message": "获取成功", "data": weather})

    except Exception as e:
        current_app.logger.exception("天气获取错误: %s", e)
        return jsonify({"success": False, "message": f"天气获取失败: {str(e)}"}), 500
...
def get_history():
    ...
    获取用户的历史分析记录
    """
    return jsonify({"success": True, "data": db.find_many("results", limit=50)})
